...
def get_device_capture(index, fps, width, height):
    ...
    # cap = VideoGear(source=index, logging=False, framerate=fps, resolution=(width,height)).start()
...
class CameraReader:
    ...
    def __init__(self, cameras, camera_sources, start_time, fps=60, is_videogear=False, simulate_fps=None, is_process=False) -> None:
        ...
        self.fps = fps
        self.is_videogear = is_videogear
        ...
        is_process = False
        if is_process:
            self.sync_barrier = multiprocessing.Barrier(len(camera_sources))
        else:
            self.sync_barrier = threading.Barrier(len(camera_sources))
        self._sync_diff_dict = {_ : list() for _ in range(len(camera_sources))}
        for source_id, source in enumerate(camera_sources.values()):
            if source is None:
                raise ValueError(f"Camera source is None")
            if not callable(source):
                raise ValueError(f"Camera source {source} is not callable")
            if is_process:
                # Multi-processing
                q = ArrayQueue(1000)
                t = multiprocessing.Process(target=self.source_thread_fucnt, args=(source_id, source, q, self.sync_barrier, self._sync_diff_dict, True))
            else:
                # Multi-threading
                q = Queue(maxsize=60 * 20)
                t = Thread(target=self.source_thread_fucnt, args=(source_id, source, q, self.sync_barrier, self._sync_diff_dict))
            self.source_threads[source] = t
            self.source_queues[source] = q
        self.last_frame_id = 0
        for t in self.source_threads.values():
            t.start()

    def source_thread_fucnt(self, source_id, source, queue, sync_barrier, _sync_diff_dict, is_need_build_source=False):
        if is_need_build_source:
            source = source()
            try:
                os.nice(-5)
        ...
        while True:
            ...
            if frame is None:
                sync_barrier.abort()
                queue.put(None)
                logger.debug(f"Cap read Fid={fid} (BarrierId={bid}) is None, broke")
                break
    ...
    def run(self):
        ...
        while True:
            ...
            for cam, source in self.camera_sources.items():
                sim_st_time = time.time()
                source_st_time = time.time()
                if self.is_videogear:
                    # frame = source()
                    frame = self.source_queues[source].get()
                else:
                    ret, frame = source()
                logger.info(f"Frame {self.last_frame_id} source read in {time.time() - source_st_time:.2f}s, FPS: {1/(time.time() - source_st_time):.2f}")
                if (self.is_videogear and frame is None) or (not self.is_videogear and not ret):
...
class ThreadDetector:
    def __init__(self, model_path, device, imgsz, input_queue, output_queue, done_syner, threads_per_device=8, det_log_level=None):
        ...
        if is_use_process:
            self.process_input_queue = torch.multiprocessing.Queue(maxsize=10)
            self.process_output_queue = torch.multiprocessing.Queue(maxsize=10)
...
class RallyTracker:
    ...
    def __init__(self, fps=60, is_video_mode=True):
        ...
        self.ball_lost_time_threshold = fps * 3
    ...
    def on_frame(self, frame_result: FrameResult):
        ...
        if self.state == RallyState.WAIT_SERVE:
            ...
            if sample_frame_result.ball_vel is not None and sample_frame_result.main_pos3d is not None and \
                ...
                if ball_vel_kmh >= 40 and -15 <= tilt <= 45 and yaxis_angle <= 30:
            ...
            if frame_result.main_pos3d is not None and frame_result.ball_vel_kmh is not None \
                    ...
                    and frame_result.ball_tilt_angle_deg is not None and frame_result.ball_tilt_angle_deg <= 20:
                # find max speed
                max_vel_kmh = max([fr.ball_vel_kmh for fr in sample_buffer if fr.ball_vel_kmh is not None])
                if max_vel_kmh >= 60:
...
class ResultRenderer:
    ...
    def __init__(self, cameras, video_size=(1920, 1080), fps=60, is_video_mode=True, is_sponsor_mode=False) -> None:
        ...
        self.frame_result_queue = Queue(maxsize=60*6)  # this is increased to >120 because the prepend is now handle by RallyTracker
        ...
        for cam in cameras:
            ...
            self.camera_render_queues[cam] = render_q = Queue(maxsize=10)
            self.camera_write_queues[cam] = write_q = Queue(maxsize=10 * self.fps)
            ...
            self.camera_render_info_cards_data_queue[cam] = data_queue = multiprocessing.Queue(maxsize=10)
        ...
        self.render_3d_court_data_queue = multiprocessing.Queue(maxsize=10)
    ...
    def render_infocards_stage_process_funct(input_frame_queue, court_plane_image_queue, court_3d_image_queue, sponsors_image_queue, output_frame_queue, data):
        ...
        right_shift = 80
        visuals.draw_result_info_card(frame, (960 - 520 + right_shift, 1080 - 160 - 20), "Serve Speed", serve_speed_kmh_str)
        visuals.draw_result_info_card(frame, (960 - 520 + right_shift, 1080 -  80 - 10), "Serve To Net", serve_cross_net_seconds_str)
        visuals.draw_result_info_card(frame, (960 +   0 + 10 + right_shift, 1080 - 160 - 20), "Spike Speed", spike_speed_kmh_str)
        visuals.draw_result_info_card(frame, (960 +   0 + 10 + right_shift, 1080 -  80 - 10), "Spike Height", spike_height_m_str)
...
class DetectSystem:
    ...
    def __init__(self, model_path, model_imgsz, cameraset, camera_name_inputs, start_time, output_dir, fps=60, video_size=(1920, 1080), 
                 is_video_mode=True, is_device_mode=False, is_write_full=False, is_display_mode=False, simulate_fps=None, is_sponsor_mode=False,
                 det_log_level=None) -> None:
        self.fps = fps
        self.video_size = video_size
        ...
        is_camera_reader_process = False
        for camera_name, input_path in camera_name_inputs.items():
            camera = self.camera_name_camera_map.get(camera_name)
            if is_device_mode:
                logger.info("Opening camera device {}", input_path)
                cap = get_device_capture(input_path, fps, video_size[0], video_size[1])
                self.camera_get_frame_functs[camera] = cap.read
            else:
                if not Path(input_path).exists() or input_path.stem.isnumeric():
                    logger.error(f"Camera {camera_name} input path {input_path} not exists or is a device index")
                    exit(-1)
                logger.info("Opening camera {}", input_path)
                # cap = cv.VideoCapture(str(input_path))
                if not is_camera_reader_process:
                    cap = VideoGear(source=str(input_path), logging=False, framerate=fps, resolution=video_size).start()
                    self.camera_get_frame_functs[camera] = cap.read
                else:
                    src = lambda: VideoGear(source=str(input_path), logging=False, framerate=fps, resolution=video_size).start().read
                    self.camera_get_frame_functs[camera] = src
        if not is_camera_reader_process:  # original threaded CameraReader
            self.camera_reader = CameraReader(self.cameras, self.camera_get_frame_functs, start_time, self.fps, is_videogear=True, simulate_fps=simulate_fps)
        else:
            self.camera_reader = CameraReader(self.cameras, self.camera_get_frame_functs, start_time, self.fps, is_videogear=True, simulate_fps=simulate_fps, \
                is_process=True)
        ...
        if is_display_mode:
            self.realtime_display = RealtimeDisplay(video_size, fps, self.cameras[-1])
